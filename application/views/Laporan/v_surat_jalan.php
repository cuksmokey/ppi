								...
								<tr>
									...
									<td colspan="3">
										<select name="" id="jenis" class="form-control" style="margin: 5px">
											...
											$sql = mysql_query("SELECT DISTINCT b.no_pkb as no,b.tgl AS tgl,b.nm_perusahaan as np,b.nama FROM m_timbangan a
                                                ...
                                                ORDER BY b.tgl DESC,b.no_pkb ASC");
											...
											?>
												...
												<option value="<?= $data['no'] ?>"><?= $data['tgl'] ?> || <?= $data['no'] ?> || <?= $nmPt ?></option>
...
<script>
	function cetak(ctk) {
		...
		if (jenis == 0) {
			showNotification("alert-info", "PILIH PACKING LIST DAHULU !!!", "top", "center", "", "");
			return;
		}
